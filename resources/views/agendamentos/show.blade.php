...
@section('content')
    ...
    <div style="background:var(--sa-surface);border-radius:12px;border:1px solid var(--sa-border);padding:20px;margin-bottom:16px">
        <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(140px,1fr));gap:20px">
            ...
            <div>
                <div style="font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:.06em;color:var(--sa-text3);margin-bottom:6px">Dura��o</div>
                <div style="font-size:15px;font-weight:600;color:var(--sa-text1)">{{ $agendamento->duracao }} min</div>
                <div style="font-size:13px;color:var(--sa-text3)">T�rmino: {{ $agendamento->data_hora->copy()->addMinutes($agendamento->duracao)->format('H:i') }}</div>
            </div>
            @if($agendamento->servico)
            <div>
                <div style="font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:.06em;color:var(--sa-text3);margin-bottom:6px">Servi�o</div>
            ...
            <div style="grid-column:1/-1;background:var(--sa-surface2);border-radius:10px;border:1px solid var(--sa-border);padding:16px;margin-top:4px">
                ...
                <div style="display:flex;align-items:center;gap:8px;font-size:13px;color:var(--sa-text2)">
                    ...
                    <span>Aprovação manual — cliente pagará <strong>R$ {{ number_format((float)$agendamento->valor, 2, ',', '.') }}</strong> integral no dia.</span>
                </div>
                @else
        ...
        <div style="padding-top:16px;margin-top:16px;border-top:1px solid var(--sa-border)">
            <div style="font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:.06em;color:var(--sa-text3);margin-bottom:6px">Observa��o</div>
...
function aprovarManualShow(id) {
    Swal.fire({
        ...
        text: 'O agendamento será confirmado sem sinal. O cliente pagará o valor integral no dia.',
        icon: 'question',
...
function confirmCancelamento(e) {
    ...
    Swal.fire({
        ...
        text: 'Esta a��o alterar� o status para "Cancelado".',
        icon: 'warning',
        showCancelButton: true,
