...
<style>
    ...
    .sa-site-nav button.active { background:color-mix(in srgb,var(--sa-primary) 8%,transparent); color:var(--sa-primary); font-weight:600; border-left-color:var(--sa-primary); }
    .sa-site-field { width:100%; padding:9px 12px; font-size:13px; border:1.5px solid var(--sa-border); border-radius:8px; background:var(--sa-surface); color:var(--sa-text1); outline:none; box-sizing:border-box; font-family:var(--sa-font-body); transition:border-color 160ms,outline 160ms; }
    ...
    @media (max-width:768px) { .sa-site-layout { grid-template-columns:1fr; } .sa-site-nav { position:static; flex-direction:row; overflow-x:auto; } }
</style>
@endpush
...
<x-sa.page x-data="{
    ...
    async handleBannerFile(e) {
        ...
        try {
            const res = await fetch('{{ route('site.upload.banner') }}', { method: 'POST', body: fd });
            if (!res.ok) throw new Error();
            const json = await res.json();
            this.bannerUrl = json.url;
            Swal.fire({ toast: true, position: 'top-end', icon: 'success', title: 'Banner enviado com sucesso!', showConfirmButton: false, timer: 2500, timerProgressBar: true });
        } catch {
            Swal.fire({ toast: true, position: 'top-end', icon: 'error', title: 'Erro ao enviar o banner.', showConfirmButton: false, timer: 2500 });
        } finally {
    ...
    async removeBanner() {
        const r = await Swal.fire({ title: 'Remover banner?', text: 'O banner atual será apagado permanentemente.', icon: 'warning', showCancelButton: true, confirmButtonText: 'Sim, remover', cancelButtonText: 'Cancelar', confirmButtonColor: '#ef4444', cancelButtonColor: 'transparent', customClass: { cancelButton: 'swal-cancel-muted' } });
        if (!r.isConfirmed) return;
        try {
            const res = await fetch('{{ route('site.remove.banner') }}', {
                method: 'DELETE',
            ...
            Swal.fire({ toast: true, position: 'top-end', icon: 'error', title: 'Erro ao remover o banner.', showConfirmButton: false, timer: 2500 });
        }
    },
    ...
    async handleOgFile(e) {
        ...
        fd.append('_token', document.querySelector('meta[name=csrf-token]').content);
        try {
            const res = await fetch('{{ route('site.upload.og') }}', { method: 'POST', body: fd });
            if (!res.ok) throw new Error();
            const json = await res.json();
            this.ogUrl = json.url;
            Swal.fire({ toast: true, position: 'top-end', icon: 'success', title: 'OG Image enviada!', showConfirmButton: false, timer: 2500, timerProgressBar: true });
        } catch {
            Swal.fire({ toast: true, position: 'top-end', icon: 'error', title: 'Erro ao enviar a imagem.', showConfirmButton: false, timer: 2500 });
        } finally {
        ...
        <div class="sa-site-layout">
            <nav class="sa-site-nav">
                ...
                <div style="margin-top:16px;padding:12px;background:color-mix(in srgb,var(--sa-secondary) 10%,transparent);border-radius:10px;border:1px solid color-mix(in srgb,var(--sa-secondary) 20%,transparent)">
                    ...
                    <div style="font-size:11px;color:var(--sa-text3);line-height:1.6">Acesse a página pública pelo botão no topo do painel</div>
                    @if($publicUrl)
            ...
            <div>
                {{-- Banner & Hero --}}
                <div x-show="tab === 'banner'" x-cloak style="display:flex;flex-direction:column;gap:20px">
                    ...
                    <x-sa.card style="padding:22px">
                        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:14px">
                            ...
                            <button type="button" role="switch" class="sa-toggle" :class="{ 'is-on': site.show_stats }" :aria-checked="site.show_stats ? 'true' : 'false'" @click="site.show_stats = !site.show_stats">
                                <span class="sa-toggle__knob"></span>
                            </button>
                        </div>
                        <div x-show="site.show_stats" style="display:grid;grid-template-columns:repeat(4,1fr);gap:10px">
                    ...
                    <x-sa.card style="padding:22px">
                        ...
                        <div x-show="!bannerUrl" class="sa-site-dropzone" @click="triggerBannerUpload()">
                            ...
                            <template x-if="uploading">
                                <div style="font-size:14px;color:var(--sa-text3)">Enviando…</div>
                            </template>
                        </div>
                    </x-sa.card>
                </div>
                ...
                <div x-show="tab === 'sections'" x-cloak>
                    <x-sa.card style="padding:22px">
                        ...
                        <div class="sa-setting-row">
                            ...
                            <button type="button" role="switch" class="sa-toggle" :class="{ 'is-on': site.{{ $sec['key'] }} }" :aria-checked="site.{{ $sec['key'] }} ? 'true' : 'false'" @click="site.{{ $sec['key'] }} = !site.{{ $sec['key'] }}">
                                <span class="sa-toggle__knob"></span>
                            </button>
                        </div>
                        @endforeach
                ...
                {{-- Mensagens --}}
                <div x-show="tab === 'messages'" x-cloak style="display:flex;flex-direction:column;gap:20px">
                    <x-sa.card style="padding:22px">
                ...
                {{-- SEO & Analytics --}}
                <div x-show="tab === 'seo'" x-cloak style="display:flex;flex-direction:column;gap:20px">
                    <x-sa.card style="padding:22px">
                    ...
                    <x-sa.card style="padding:22px">
                        ...
                        <div x-show="!ogUrl" class="sa-site-dropzone sa-site-dropzone--sm" @click="triggerOgUpload()">
                            ...
                            <template x-if="uploadingOg">
                                <div style="font-size:13px;color:var(--sa-text3)">Enviando…</div>
                            </template>
                        </div>
                    </x-sa.card>
