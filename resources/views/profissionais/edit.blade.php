...
@section('content')
    ...
    <div style="background:var(--sa-surface);border-radius:12px;border:1px solid var(--sa-border);padding:28px" x-data="editProfApp()">
        <form method="POST" action="{{ route('profissionais.update', $profissional) }}" style="display:flex;flex-direction:column;gap:18px">
            @csrf
            @method('PUT')

            <input type="hidden" name="cor" :value="cor">

            @php $vinculados = $profissional->servicos->pluck('id')->map(fn($id) => (string)$id)->toArray(); @endphp

            <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px">
                {{-- Color picker --}}
                <div style="grid-column:1/-1;padding:16px;background:var(--sa-surface2);border-radius:12px;border:1px solid var(--sa-border);display:flex;align-items:center;gap:16px">
                    <div style="width:56px;height:56px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:20px;font-weight:800;font-family:'Inter',sans-serif;color:#fff;flex-shrink:0"
                         :style="'background:' + cor">{{ strtoupper(substr($profissional->name, 0, 1)) }}</div>
                    <div style="flex:1">
                        <div style="font-size:13px;font-weight:600;color:var(--sa-text1);margin-bottom:8px">Cor do profissional</div>
                        <div style="display:flex;gap:7px;flex-wrap:wrap">
                            <template x-for="c in profColors" :key="c">
                                <button type="button" @click="cor = c"
                                        :style="'width:22px;height:22px;border-radius:50%;background:' + c + ';border:' + (cor === c ? '3px solid var(--sa-text1)' : '2px solid transparent') + ';cursor:pointer;transition:border 150ms'"></button>
                            </template>
                        </div>
                    </div>
                </div>
                ...
                <div style="grid-column:1/-1">
                    ...
                    <input type="text" name="name" value="{{ old('name', $profissional->name) }}" required placeholder="Nome do profissional"
                ...
                <div>
                    ...
                    <input type="text" name="especialidade" value="{{ old('especialidade', $profissional->especialidade) }}" placeholder="Ex: Barbeiro, Cabeleireiro, Manicure"
                ...
                <div>
                    ...
                    <input type="range" name="comissao_pct" x-model.number="comissao" min="0" max="70" step="1"
                           style="width:100%;accent-color:var(--sa-primary);cursor:pointer;margin-top:4px">
                    <div style="display:flex;justify-content:space-between;font-size:11px;color:var(--sa-text3);margin-top:3px">
                        <span>0%</span>
                        <span style="font-weight:700;color:var(--sa-secondary)" x-text="comissao + '%'"></span>
                        <span>70%</span>
                    </div>
                ...
                <div>
                    ...
                    <input type="tel" name="phone" value="{{ old('phone') ? \App\Support\PhoneFormatter::format(old('phone')) : \App\Support\PhoneFormatter::format($profissional->phone) }}" data-sa-phone-mask maxlength="15" placeholder="(11) 99999-0000"
                           style="width:100%;padding:10px 12px;border:1.5px solid var(--sa-border);border-radius:8px;font-size:14px;color:var(--sa-text1);background:var(--sa-surface);outline:none;transition:border-color 180ms,outline 180ms"
                           onfocus="this.style.borderColor='var(--sa-primary)';this.style.outline='3px solid rgba(0,0,0,.06)'" onblur="this.style.borderColor='var(--sa-border)';this.style.outline='none'">
                </div>
                ...
                <div>
                    ...
                    <input type="date" name="admissao" value="{{ old('admissao', $profissional->admissao?->format('Y-m-d')) }}"
                           style="width:100%;padding:10px 12px;border:1.5px solid var(--sa-border);border-radius:8px;font-size:14px;color:var(--sa-text1);background:var(--sa-surface);outline:none;transition:border-color 180ms,outline 180ms"
                           onfocus="this.style.borderColor='var(--sa-primary)';this.style.outline='3px solid rgba(0,0,0,.06)'" onblur="this.style.borderColor='var(--sa-border)';this.style.outline='none'">
                </div>
            </div>
...
@push('scripts')
<script>
function editProfApp() {
    return {
        comissao: {{ (float)($profissional->comissao_pct ?? 0) }},
        cor: '{{ old('cor', $profissional->cor ?? '#1a1a1a') }}',
        profColors: ['#1a1a1a','#d4a574','#6366f1','#10b981','#f59e0b','#ec4899','#ef4444','#0ea5e9','#8b5cf6','#14b8a6'],
    };
}
</script>
@endpush
